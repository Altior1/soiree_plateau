defmodule SoireePlateauWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in - email and password" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_soiree_plateau_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
    ...
    test "logs the user in with return to", %{conn: conn, user: user} do
      ...
      assert Phoenix.Flash.get(conn.assigns.flash, :info) =~ "Bon retour !"
    end
  ...
  describe "POST /users/log-in - magic link" do
    ...
    test "confirms unconfirmed user", %{conn: conn, unconfirmed_user: user} do
      ...
      assert get_session(conn, :user_token)
      assert redirected_to(conn) == ~p"/"
      assert Phoenix.Flash.get(conn.assigns.flash, :info) =~ "Utilisateur confirmé avec succès."
