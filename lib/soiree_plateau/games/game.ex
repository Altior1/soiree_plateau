defmodule SoireePlateau.Games.Game do
  ...
  def changeset(game, attrs) do
    game
    |> cast(attrs, [:name, :description, :image_url, :nb_players_min, :nb_players_max, :duration, :complexity])
    |> validate_required([:name, :description, :nb_players_min, :nb_players_max, :duration, :complexity])
  ...
  defp validate_nb_players(changeset) do
    ...
    if nb_players_min && nb_players_max && nb_players_min > nb_players_max do
      add_error(changeset, :nb_players_min, "must be less than or equal to maximum number of players")
    else
      changeset
    end
  end
